...
- **`x, y, z`**: Represent the position or magnitude along the 3 axes.
...
A `Canvas` is a rectangular grid of pixels, where each pixel is a `Color`. It represents the final image that is generated by the ray tracer. Once rendering is complete, the `Canvas` is typically exported to a standard image format like PPM (Portable Pixmap).
