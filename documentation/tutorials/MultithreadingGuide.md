...
In your main entry Point file (where the main rendering loop occurs, such as `visualizers/3.SphereShadow.cpp`), you need to uncomment or add an OpenMP pragma directive to tell the compiler to parallelize the loop.

As an example, in `visualizers/3.SphereShadow.cpp`, uncomment / add the following line just above the outer `for` loop that iterates over the Canvas pixels: `#pragma omp parallel for`
```c++
